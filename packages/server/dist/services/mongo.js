...
function getMongoURI(dbname) {
  ...
  console.log("Connecting to MongoDB at", connection_string.replace(/:[^@]+@/, ":<password>@"));
  return connection_string;
}
function connect(dbname) {
  import_mongoose.default.connect(getMongoURI(dbname), { useNewUrlParser: true, useUnifiedTopology: true }).then(() => console.log("Connected to MongoDB")).catch((error) => console.error("Error connecting to MongoDB:", error));
}
